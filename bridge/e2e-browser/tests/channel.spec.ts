...
test('添加 Mattermost 账号 - 关键验证：不卡死', async ({ page }) => {
  ...
  // 填写表单 - 使用固定 ID 避免冲突
  await page.locator('.el-form-item', { hasText: '账号 ID' }).locator('input').fill('test-account-001');
  await page.locator('.el-form-item', { hasText: '显示名称' }).locator('input').fill('Test Account');
