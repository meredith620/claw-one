...
fn get_test_modules() -> Vec<(
    &'static str,
    &'static str,
    serde_json::Value,
    fn(&serde_json::Value) -> Option<&serde_json::Value>,
)> {
    vec![
        (
            "memory",
        ...
        (
            ...
            |file_json: &serde_json::Value| file_json.get("agents"),
        ),
        (
            "channels",
            "/api/channels",
...
async fn test_all_modules_get_returns_saved_data() {
    ...
    for (name, path, valid_config, _) in &modules {
        ...
        // 2. GET 验证
        let get_response = server.client.get(server.url(*path)).send().await.unwrap();
