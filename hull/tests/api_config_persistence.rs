...
async fn test_memory_config_file_content_matches_api_save() {
    ...
    assert_eq!(api_memory["model"], "test-memory-model-unique-id", "API 返回的 model 应该匹配");
    ...
    let file_json: serde_json::Value = serde_json::from_str(&file_content)
        .expect("openclaw.json 应该是合法的 JSON");
