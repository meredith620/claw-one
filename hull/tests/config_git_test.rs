...
mod tests {
    ...
    async fn create_test_manager_with_git() -> (TempDir, ConfigManager) {
        ...
        tokio::fs::create_dir_all(&version_config_dir)
            .await
            .unwrap();
        ...
        let _ = Command::new("git")
            .args([
                "-C",
                version_config_dir.to_str().unwrap(),
                "config",
                "user.email",
                "test@test.com",
            ])
    ...
    async fn test_sync_to_version_config_creates_commit() {
        let (temp_dir, manager): (TempDir, ConfigManager) = create_test_manager_with_git().await;
        ...
        // 保存并同步到 Git
        let result: Result<Option<String>, _> = manager
            .sync_to_version_config(&new_config, Some("Test commit".to_string()))
            .await;
        ...
        assert!(
            ...
            "Expected 'Test commit' in git log, got: {}",
            log
        );
    }
    ...
    async fn test_sync_to_version_config_no_changes() {
        let (_temp_dir, manager): (TempDir, ConfigManager) = create_test_manager_with_git().await;
        ...
        // 先同步一次
        let _: Option<String> = manager
            .sync_to_version_config(&same_config, Some("First commit".to_string()))
            .await
            .unwrap();

        // 再次同步相同内容
        let result: Result<Option<String>, _> = manager
            .sync_to_version_config(&same_config, Some("Second commit".to_string()))
            .await;
    ...
    async fn test_list_snapshots() {
        ...
        assert!(
            snapshots[0].message.contains("Initial")
                || snapshots[0].message.contains("Initial commit")
        );
        ...
        for i in 1..=3 {
            ...
            manager
                .sync_to_version_config(&new_config, Some(format!("Commit {}", i)))
                .await
                .unwrap();
        }
        ...
        // 验证按时间倒序排列（最新的在前）
        assert!(
            snapshots[0].message.contains("Commit 3") || snapshots[0].message.contains("Initial")
        );
    }
    ...
    async fn test_rollback_to_previous_version() {
        ...
        manager
            .sync_to_version_config(&new_config, Some("Second commit".to_string()))
            .await
            .unwrap();
    ...
    async fn test_rollback_preserves_git_history() {
        ...
        assert_eq!(count_before, 4, "Should have 4 snapshots before rollback");

        // 回滚到第二个提交
        let second_commit = &snapshots_before[snapshots_before.len() - 2].id;
        manager.rollback(second_commit).await.unwrap();

        // 验证 Git 历史仍然完整（回滚可能创建新 commit，所以 >= count_before）
        let snapshots_after = manager.list_snapshots().await.unwrap();
        assert!(
            snapshots_after.len() >= count_before,
            "Rollback should not delete git history, before={}, after={}",
            count_before,
            snapshots_after.len()
        );
    }
    ...
    async fn test_apply_config_creates_git_commit() {
        ...
        let result: Result<String, _> = manager
            .apply_config(new_config, Some("Apply new config".to_string()))
            .await;
    ...
    async fn test_git_with_special_characters_in_message() {
        ...
        let result: Result<Option<String>, _> = manager
            .sync_to_version_config(&new_config, Some(message.to_string()))
            .await;
    ...
    async fn test_concurrent_git_operations() {
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                ...
                mgr.sync_to_version_config(&new_config, Some(format!("Concurrent commit {}", i)))
                    .await
            });
            handles.push(handle);
        }
        ...
        // 等待所有任务完成
        let results: Vec<Result<Result<Option<String>, _>, _>> =
