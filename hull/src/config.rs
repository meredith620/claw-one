...
impl ConfigManager {
    ...
    fn get_openclaw_home_from_settings() -> String {
        ...
        eprintln!(
            "[DEBUG] ConfigManager: 尝试读取 claw-one.toml: {}",
            config_file
        );
        ...
        eprintln!(
            "[DEBUG] ConfigManager: 使用默认 openclaw_home = {}",
            default_path
        );
        default_path
    }
    ...
    pub async fn migrate_legacy_git_repo(&self) -> Result<bool> {
        ...
        if !self.version_config_dir.exists() {
            fs::create_dir_all(&self.version_config_dir)
                .await
                .map_err(|e| {
                    AppError::Runtime(format!("Failed to create version-config dir: {}", e))
                })?;
        }

        // 2. 复制当前配置文件到 version-config/（如果不存在）
        if self.config_path.exists() && !self.version_config_path.exists() {
            fs::copy(&self.config_path, &self.version_config_path)
                .await
        ...
        let output = Command::new("git")
            .args(["-C", self.version_config_dir.to_str().unwrap(), "add", "-A"])
            .output()
        ...
        if !status_output.is_empty() {
            ...
            let output = Command::new("git")
                .args([
                    ...
                    "commit",
                    "-m",
                    "Migrate to version-config layout",
                ])
                .output()
    ...
    async fn copy_dir_recursive(src: &PathBuf, dst: &PathBuf) -> Result<()> {
            ...
            .map_err(|e| AppError::Runtime(format!("Failed to read entry: {}", e)))?
        {
            ...
            if entry
                .file_type()
                .await
                .map_err(|e| AppError::Runtime(format!("Failed to get file type: {}", e)))?
                .is_dir()
            {
    ...
    pub async fn ensure_git_repo(&self) -> Result<()> {
        ...
        if !self.version_config_dir.exists() {
            fs::create_dir_all(&self.version_config_dir)
                .await
                .map_err(|e| {
                    AppError::Git(format!("Failed to create version-config dir: {}", e))
                })?;
        }
    ...
    pub async fn get_config(&self) -> Result<Config> {
        ...
        eprintln!(
            "[DEBUG] ConfigManager: 尝试读取配置 from {:?}",
            self.config_path
        );
        ...
        eprintln!(
            "[DEBUG] ConfigManager: 文件内容长度: {} bytes",
            content.len()
        );
    ...
    pub async fn apply_config(&self, config: Config, message: Option<String>) -> Result<String> {
        // 使用 sync_to_version_config 完成完整流程
        let commit_msg =
            message.unwrap_or_else(|| format!("Config update at {}", Utc::now().to_rfc3339()));
    ...
    pub async fn sync_to_version_config(
        &self,
        config: &Config,
    ...
    /// 保存 Provider 实例
    pub async fn save_provider(&self, provider_id: &str, data: &serde_json::Value) -> Result<()> {
    ...
    /// 保存模型优先级
    pub async fn save_model_priority(&self, primary: &str, fallbacks: &[String]) -> Result<()> {
        let mut config = self.get_config().await?;
        ...
        config["agents"]["defaults"]["model"]["fallbacks"] = serde_json::Value::Array(
            fallbacks
                .iter()
    ...
    pub async fn get_agents(&self) -> Result<serde_json::Value> {
        ...
        let agents = config.get("agents").cloned().unwrap_or_else(|| {
            serde_json::json!({
                "defaults": {
            ...
            if let Some(new_defaults) = new_obj.get("defaults") {
                if let Some(new_def_obj) = new_defaults.as_object() {
                    ...
                    let mut merged_defaults =
                        if let Some(existing_defaults) = existing_obj.get("defaults") {
                    ...
                    result.insert(
                        "defaults".to_string(),
                        serde_json::Value::Object(merged_defaults),
                    );
                }
            }
    ...
    pub async fn get_channels(&self) -> Result<serde_json::Value> {
        ...
        let channels = config.get("channels").cloned().unwrap_or_else(|| {
            serde_json::json!({
                "mattermost": {
                    "enabled": false,
                    "dmPolicy": "pairing",
...
mod tests {
    ...
    async fn test_get_config_valid() {
        ...
        fs::write(&manager.config_path, config.to_string())
            .await
            .unwrap();
    ...
    async fn test_get_config_invalid_json() {
        ...
        // 写入不合法的 JSON
        fs::write(&manager.config_path, "{ invalid json")
            .await
            .unwrap();
    ...
    async fn test_get_providers_with_data() {
        ...
        assert_eq!(
            providers[0].get("id").unwrap().as_str().unwrap(),
            "test-provider"
        );
    }
    ...
    async fn test_save_provider_new() {
        ...
        manager
            .save_provider("new-provider", &provider_data)
            .await
            .unwrap();
    ...
    async fn test_save_provider_update() {
        ...
        manager
            .save_provider("existing", &provider_data)
            .await
            .unwrap();
        ...
        let config = manager.get_config().await.unwrap();
        assert_eq!(
            config["models"]["providers"]["existing"]["apiKey"]
                .as_str()
                .unwrap(),
            "sk-new"
        );
        assert_eq!(
            config["models"]["providers"]["existing"]["api"]
                .as_str()
                .unwrap(),
            "anthropic-messages"
        );
    ...
    async fn test_save_memory() {
        ...
        let config = manager.get_config().await.unwrap();
        assert_eq!(
            config["agents"]["defaults"]["memorySearch"]["model"]
                .as_str()
                .unwrap(),
            "test-model"
        );
    ...
    async fn test_get_channels_default() {
        ...
        assert_eq!(channels["mattermost"]["enabled"].as_bool().unwrap(), false);
    }
